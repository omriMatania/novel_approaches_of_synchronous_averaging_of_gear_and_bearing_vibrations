...
records_path = ['D:\data\papers\synchronous_average_for_bearing_vibrations\endurance\',axis_name,'\'] ;
...
for record_num = 1:num_records
    ...
    cyc = [0:dcyc:(N-1)*dcyc].';
    subplot(2,2,ind)
    plot(cyc, circshift(sas_angulr_sync(:, 1), -100), 'LineWidth', 2)
    hold on
    plot(cyc, circshift(sas_angulr_sync(:, 2), -100), 'LineWidth', 1)
    hold off
    ylim(y_lim_range)
    ax = gca;
