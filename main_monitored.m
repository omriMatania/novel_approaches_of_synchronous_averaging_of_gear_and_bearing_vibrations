...
records_path = ['D:\data\papers\synchronous_average_for_bearing_vibrations\monitored\',axis_name,'\'] ;

if strcmp(axis_name, 'Z')
    y_lim_range = [-2.4, 2.6] ;
elseif strcmp(axis_name, 'Y')
    y_lim_range = [-4.1, 4.1] ;
end

num_sgmnts_for_sa = 500 ;
num_sas = 10 ;
...
for record_num = 1:num_records
    ...
    if record_num > 1
        [sa_new, d] = sync_sigs(sas_angulr_sync(:,1), sa_1) ;
        sas_angulr_sync(:,1) = sa_new ;
    else
        sa_1 = sas_angulr_sync(:,1) ;
    end
    ...
    cyc = [0:dcyc:(N-1)*dcyc].';
    subplot(2,2,ind)
    plot(cyc, circshift(sas_angulr_sync(:, 1), 10), 'LineWidth', 2)
    hold on
    plot(cyc, circshift(sas_angulr_sync(:, 2), 10), 'LineWidth', 1)
    hold off
    ylim(y_lim_range)
    ax = gca;
    ax.FontSize = font_axis_size; % font size.
    title(['Spall number ', num2str(ceil(record_num/5)), ', record number ', num2str(record_num)], ...
